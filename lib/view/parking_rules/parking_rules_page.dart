...
import 'package:flutter_ehliyet_sinavi/view/parking_rules/parking_rules_details/parking_rules_details_page.dart';
...
class ParkingRulesPage extends BaseGetView<ParkingRulesPageController> {
  const ParkingRulesPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: StringConstants.parkingRules),
      body: SafeArea(
        child: SizedBox(
          width: Get.width,
          height: Get.height,
  ...
  Widget _buildCard({required ParkingRulesModel model}) {
    return InkWell(
      onTap: () {
        Get.to(() => ParkingRulesDetailsPage(model: model));
      },
      child: Container(
        width: Get.width,
          ...
          children: [
            Image(
              image: const AssetImage(
                AssetsConstants.carParkingIcon,
              ),
              fit: BoxFit.cover,
              width: Get.width * 0.1,
