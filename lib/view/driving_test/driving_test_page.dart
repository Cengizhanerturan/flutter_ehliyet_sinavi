...
import 'package:flutter_ehliyet_sinavi/view/driving_test/driving_test_details/driving_test_details_page.dart';
...
class DrivingTestPage extends BaseGetView<DrivingTestPageController> {
  const DrivingTestPage({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(title: StringConstants.drivingTest),
      body: SafeArea(
        child: SizedBox(
          width: Get.width,
          height: Get.height,
  ...
  Widget _buildCard({required DrivingTestModel model}) {
    return InkWell(
      onTap: () {
        Get.to(() => DrivingTestDetailsPage(model: model));
      },
      child: Container(
        width: Get.width,
