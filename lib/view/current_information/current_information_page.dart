...
import 'package:flutter_ehliyet_sinavi/view/current_information/current_information_details/current_information_details_page.dart';
...
class CurrentInformationPage
    extends BaseGetView<CurrentInformationPageController> {
  const CurrentInformationPage({super.key});
  ...
  Widget _buildCard({required CurrentInformationModel model}) {
    return InkWell(
      onTap: () {
        Get.to(() => CurrentInformationDetailsPage(model: model));
      },
      child: Container(
        width: Get.width,
          ...
          children: [
            Image(
              image: const AssetImage(
                AssetsConstants.informationIcon,
              ),
              fit: BoxFit.cover,
              width: Get.width * 0.1,
